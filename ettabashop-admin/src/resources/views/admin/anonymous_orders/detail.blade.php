    ...
    <section class="invoice">
        ...
        <div class="row invoice-info">
            ...
            <div class="col-sm-4 invoice-col">
                ...
                <b>Payment status:</b>
                @if(($anonymous_order->paymentMethod->short_code ?? '') === 'EPS')
                    @if(($anonymous_order->payment_status ?? '') === 'paid')
                        <span class="label label-success">Gateway Payment</span>
                    @else
                        <span class="label label-warning">Pending (EPS)</span>
                    @endif
                @else
                    <span class="label label-info">{{ strtoupper($anonymous_order->payment_status ?? 'DUE') }} ({{ $anonymous_order->paymentMethod->short_code ?? 'COD' }})</span>
                @endif
                <br>
                @if(($anonymous_order->paymentMethod->short_code ?? '') === 'EPS' && ($anonymous_order->payment_status ?? '') !== 'paid')
                    <a href="{{ route('order.verifyEps', $anonymous_order->id) }}" class="btn btn-xs btn-info" style="margin-top: 5px; margin-bottom: 5px;">
                ...
                <table class="table table-striped">
                    ...
                    @foreach($anonymous_order->anonymousOrderItems as $item)
                        <tr>
                            ...
                            <td><a href="{{ route('anonymousitem.remove',['id'=>$item->id,'product_id'=>$item->product->id,'order_id'=>$item->order_id]) }}"
        ...
        <div class="row">
            ...
            <div class="col-xs-6">
                ...
                <h4> {{ $anonymous_order->paymentMethod->short_code }} (<span style="font-size: 15px">{{ $anonymous_order->paymentMethod->name }}</span>)   </h4>
