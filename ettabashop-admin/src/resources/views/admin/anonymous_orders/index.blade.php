            ...
            <div class="col-md-12">
                ...
                <!-- /.box-header -->
                    ...
                    <div class="box-body">
                        ...
                        <div class="col-md-6">
                            <a href="{{ route('anonymousOrder.index',['state'=>'new']) }}" class="btn btn-sm btn-success">New
                                Orders</a>
                            <a href="{{ route('anonymousOrder.index',['state'=>'']) }}" class="btn btn-sm btn-success">Orders
                                today</a>
                            <a href="{{ route('anonymousOrder.index',['state'=>'all']) }}" class="btn btn-sm btn-success">All
                                Orders</a>
                        </div>
                    </div>
                    <!-- /.box-body -->
                    <div class="box-footer">
                        <table id="orderTable" class="table table-bordered table-hover">
                            ...
                            <tbody>
                            @if($anonymous_orders)
                                @foreach($anonymous_orders as $order)
                                    <tr>
                                        <td>{{ $order->id }}</td>
                                        ...
                                        <td class="text-center">
                                            @if($order->status=='pending')
                                                <a href="{{ route('anonymousOrder.changeStatus',['id'=>$order->id,'status'=>'accepted']) }}"
                                                   class="btn btn-xs label label-sm label-default ">Accept</a>
                                                <a href="{{ route('anonymousOrder.changeStatus',['id'=>$order->id,'status'=>'canceled']) }}"

                                                   class="btn btn-xs label label-sm label-danger ">Cancel</a>
                                            @elseif($order->status=='accepted')
                                                <a href="{{ route('anonymousOrder.changeStatus',['id'=>$order->id,'status'=>'on_delivery']) }}"
                                                   class="btn btn-xs label label-sm label-warning ">On Delivery</a>
                                                <a href="{{ route('anonymousOrder.changeStatus',['id'=>$order->id,'status'=>'canceled']) }}"

                                                   class="btn btn-xs label label-sm label-danger ">Cancel</a>
                                            @elseif($order->status=='on_delivery')
                                                <a href="{{ route('anonymousOrder.changeStatus',['id'=>$order->id,'status'=>'delivered']) }}"
                                        ...
                                        <td class="text-center">
                                            @if($order->status=='delivered')
                                                <a href="{{ route('anonymousOrder.done',['id'=>$order->id]) }}"
