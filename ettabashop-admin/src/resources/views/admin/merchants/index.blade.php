            ...
            <div class="col-md-12">
                <div class="box box-info">
                    ...
                    <div class="box-body">
                        ...
                        <div class="col-md-6">
                            <a href="{{ route('merchant.index',['state'=>'all']) }}" class="btn btn-sm btn-info">All
                                Shops</a>
                            <a href="{{ route('merchant.index',['state'=>'new']) }}" class="btn btn-sm btn-success">Latest
                                Shops</a>
                            <a href="{{ route('merchant.index',['state'=>'active']) }}" class="btn btn-sm btn-success">Active
                                Shops</a>
                            <a href="{{ route('merchant.index',['state'=>'inactive']) }}"
                               class="btn btn-sm btn-success">InActive Merchants</a>
                            ...
                            @foreach($merchants as $merchant)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        @elseif($merchant->is_new==1)
                                            <span class="label label-sm label-primary">New</span>
                                        @elseif($merchant->is_blocked==1)
                                    ...
                                    {{--<td>{{ date('y-m-d',strtotime($merchant->created_at)) }}</td>--}}
                                    <td class="text-center">
                                        @if($merchant->is_active==1&& $merchant->is_new==0 && $merchant->is_blocked==0)
                                            <a href="{{ route('merchant.changeStatus',['id'=>$merchant->id,'status'=>'inactivate']) }}"
                                               class="btn btn-xs label label-sm label-default ">Mark Inactive</a>
                                            <a href="{{ route('merchant.changeStatus',['id'=>$merchant->id,'status'=>'block']) }}"
                                               class="btn btn-xs label label-sm label-danger ">Block</a>
                                        @elseif($merchant->is_active==0&& $merchant->is_new==0 && $merchant->is_blocked==0)
                                            <a href="{{ route('merchant.changeStatus',['id'=>$merchant->id,'status'=>'activate']) }}"
                                               class="btn btn-xs label label-sm label-default ">Activate</a>
                                            <a href="{{ route('merchant.changeStatus',['id'=>$merchant->id,'status'=>'block']) }}"
                                               class="btn btn-xs label label-sm label-danger ">Block</a>
                                        @elseif($merchant->is_active==0&& $merchant->is_new==1 && $merchant->is_blocked==0)
                                            <a href="{{ route('merchant.changeStatus',['id'=>$merchant->id,'status'=>'activate']) }}"
                                               class="btn btn-xs label label-sm label-default ">Activate</a>
                                            <a href="{{ route('merchant.changeStatus',['id'=>$merchant->id,'status'=>'block']) }}"
                                               class="btn btn-xs label label-sm label-danger ">Block</a>
                                        @elseif($merchant->is_active==0&& $merchant->is_new==0 && $merchant->is_blocked==1)
                                            <a href="{{ route('merchant.changeStatus',['id'=>$merchant->id,'status'=>'activate']) }}"
...
@section('extra-script')
    <script>
        $("#state").change(function () {
            ...
            window.location = '{{route('merchant.index')}}' + '?state=' + state;
            {{--var state = $(this).val();--}}
