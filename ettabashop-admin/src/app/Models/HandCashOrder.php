...
class HandCashOrder extends Model
{
    use HasFactory;
    protected $fillable = [
        'user_id',
    ...
    {
        return $this->hasMany(HandCashOrderItem::class, 'hand_cash_order_id');
    }
    ...
    {
        return Order::with('user.profile','paymentMethod')->where('orders.status',$status)->where('orders.created_at','>=',Carbon::today())->orderByDesc('id')->get();

    }
    public function GetOrderByStatus($status)
    {
        return Order::with('user.profile','paymentMethod')->where('orders.status',$status)->orderByDesc('id')->get();
    }
    public function GetDetail($id)
    {
        return Order::with('user.profile','paymentMethod','orderItems.product','orderItems.product.owner.shop')->where('orders.id',$id)->first();

    }
    public function GetOrderByUser()
    {
        return Order::with('paymentMethod','address','orderItems.product','status')->where('user_id',Auth::user()->id)->orderByDesc('id')->get();
...
//        $order = Order::with('orderItems.product','deliveryCharge')->find($id);
        $order = HandCashOrder::with('orderItems.product')->find($id);
        ...
        if ($order->orderItems!=null){
            foreach ($order->orderItems as $item)
            {
                ...
                $orderAmount+= (($item->product->price-$item->product->discount)*$item->quantity);
